...
static NTSTATUS NTAPI notify_callout(
...
{
    const FWPM_PROVIDER_CONTEXT1* context;
...
static NTSTATUS register_callout(
    const GUID* key,
    FWPS_CALLOUT_CLASSIFY_FN1 classify,
    UINT32* callout_id)
{
    ...
    callout.notifyFn = notify_callout;
    return FwpsCalloutRegister1(control_device, &callout, callout_id);
}
