...
pub(crate) fn metrics(handle: i64) -> String {
    ...
    serde_json::json!({
        ...
        "migrationReceiveFailures": metrics.migration_receive_failures.load(Ordering::Relaxed),
        "migrationWireFailures": metrics.migration_wire_failures.load(Ordering::Relaxed),
        "migrationDatagramFailures": metrics.migration_datagram_failures.load(Ordering::Relaxed),
        "migrationInnerFailures": metrics.migration_inner_failures.load(Ordering::Relaxed),
        "migrationSuccesses": metrics.migration_successes.load(Ordering::Relaxed),
