...
pub(crate) fn connect(
    ...
    wire: &ClientWire,
) -> Result<(UdpTransport, TunnelDataPlane, SessionParameters), AppleClientError> {
    let local = match (config.server, local_address) {
        (SocketAddr::V4(_), Some(IpAddr::V4(address))) => SocketAddr::from((address, 0)),
        (SocketAddr::V6(_), Some(IpAddr::V6(address))) => SocketAddr::from((address, 0)),
        (SocketAddr::V4(_), None) => SocketAddr::from(([0, 0, 0, 0], 0)),
        (SocketAddr::V6(_), None) => SocketAddr::from(([0_u16; 8], 0)),
        _ => {
            return Err(AppleClientError::new(
                "local and server address families differ",
            ))
        }
    };
    let mut transport = UdpTransport::bind(local, config.server)
        .map_err(AppleClientError::from)
        .map_err(|error| error.context("UDP connection failed"))?;
...
fn negotiate(
    ...
    config: &ValidatedClientConfig,
    wire: &ClientWire,
) -> Result<(TunnelDataPlane, SessionParameters), AppleClientError> {
    ...
    loop {
        let now = Instant::now();
        if now >= deadline {
            return Err(AppleClientError::new("handshake timed out"));
        }
...
fn send_prelude(transport: &mut UdpTransport, wire: &ClientWire) -> Result<(), AppleClientError> {
...
fn random_session_id() -> Result<u64, AppleClientError> {
