...
fn run_dns_watcher(
    ...
    while !stopping.load(Ordering::Acquire) {
        match handle.recv(&mut buffer, &mut address) {
            Ok(Some(length)) => {
                observe_dns_response(&buffer[..length], router, table);
            }
            Ok(None) => return,
            Err(error) => {
...
/// Extracts A/AAAA answers for the first DNS question and gives them the same
/// direct lifetime as the resolver advertised with the answer.
///
/// The watcher deliberately handles only conventional DNS over UDP/53. If the
/// machine uses DoH/DoT, the response is opaque here; the static GeoIP list
/// still applies, but domain-to-IP correlation is unavailable.
fn observe_dns_response(bytes: &[u8], router: &GeoRouter, table: &FlowTable) {
    let Some((payload, _source, _destination)) = udp_payload(bytes) else {
        return;
    };
    if payload.len() < 12 {
        return;
    ...
    if !router.is_direct_domain(&domain) {
        return;
    }
...
fn udp_payload(bytes: &[u8]) -> Option<(&[u8], u16, u16)> {
    let version = bytes.first()? >> 4;
    let (transport, source, destination) = match version {
        4 => {
    ...
    (bytes.len() >= payload).then_some((&bytes[payload..], source, destination))
}

/// Reads one DNS name, including compressed names.
///
/// Compression pointers are followed only to validate and advance the current
/// cursor. A small hop limit prevents malformed packets from looping forever.
